...
#define SAMPLING_FREQ   17000         // Hz, must be 40000 or less due to ADC conversion time. Determines maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
#define AMPLITUDE       150          // Depending on your audio source level, you may need to alter this value. Can be used as a 'sensitivity' control.
#define AUDIO_IN_PIN    A0            // Signal in on this pin
#define LED_PIN         0             // LED strip data
...
#define NOISE           300           // Used as a crude noise filter, values below this are ignored
...
// FastLED_NeoMaxtrix - see https://github.com/marcmerlin/FastLED_NeoMatrix for Tiled Matrixes, Zig-Zag and so forth   //NEO_MATRIX_TOP  for  16x16 metrix   11x16  NEO_MATRIX_BOTTOM
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
   ...
   /*// 8 bands, 12kHz top band
      ...
      if (i>11  && i<=14 )  bandValues[8]  += (int)vReal[i];
      if (i>14  && i<=19 )  bandValues[9]  += (int)vReal[i];
      if (i>19  && i<=24 )  bandValues[10] += (int)vReal[i];
      if (i>24  && i<=32 )  bandValues[11] += (int)vReal[i];
